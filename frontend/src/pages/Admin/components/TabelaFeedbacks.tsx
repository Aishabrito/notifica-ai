...
export default function FeedbacksTable({ feedbacks }: { feedbacks: Feedback[] }) {
                ...
                <React.Fragment key={fb._id}>
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5 max-w-sm">
                      <span className="text-xs text-slate-500 truncate block">{fb.mensagem}</span>
                    </td>
                    <td className="px-5 py-3.5">
                      {fb.lido ? (
                        <span className="inline-flex items-center text-[10px] font-medium px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 border border-slate-200">lido</span>
                      ) : (
                        <span className="inline-flex items-center text-[10px] font-semibold px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-600 border border-indigo-100">novo</span>
                      )}
                    </td>
                  </tr>
                  {isExp && (
                    <tr className="bg-slate-50/70">
                      <td colSpan={4} className="px-5 py-3">
                        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
                          ...
                          <p className="text-xs text-slate-700 leading-relaxed">{fb.mensagem}</p>
                        </div>
                      </td>
                    </tr>
                  )}
