...
export default function TabelaUsuarios({ users, alerts }: { users: UserRecord[]; alerts: AlertRecord[] }) {
  ...
  const filtered = useMemo(() => {
    return users.filter(u => !query || u.email.toLowerCase().includes(query.toLowerCase()));
  }, [users, query]);
    ...
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 gap-3 flex-wrap">
        <div>
          ...
          <p className="text-[11px] text-slate-400 mt-0.5">{filtered.length} registros</p>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100">
              ...
              <th className="px-5 py-3 text-left text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Alertas</th>
            ...
            {filtered.map(user => {
              ...
              return (
                <React.Fragment key={user._id}>
                  <tr onClick={() => setExpanded(isExp ? null : user._id)} className="hover:bg-indigo-50/30 cursor-pointer transition-colors">
                    ...
                    <tr className="bg-slate-50/70">
                      <td colSpan={3} className="px-5 py-3">
                        <div className="bg-white border rounded-xl p-4 shadow-sm space-y-2">
                          {userAlerts.length === 0 ? (
                            <p className="text-xs text-slate-400">Nenhum alerta ativo.</p>
                          ) : (
                            userAlerts.map(a => (
                              <div key={a._id} className="flex items-center gap-2">
                                <StatusBadge status={a.status} />
                                <span className="text-[11px] truncate text-indigo-600 flex-1">{a.url}</span>
                                <span className="text-[10px] text-slate-400">{fmt(a.ultimaVerificacao)}</span>
                              </div>
                            ))
                          )}
                        </div>
